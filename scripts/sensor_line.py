...
class SensorOnLine():
	...
	def recordMeas(self, msg, callback_args):
		...
		if self.lineXY.hough[optic_id] != 0 and self.lineXY.lclzn[optic_id] != 0:
			print str(loc_belt)
			if len(loc_belt) != 0 and max(loc_belt) != 0 and abs(old_belt_min - min(loc_belt)) < 0.03:
				rospy.set_param('/surf_vel_pub/belt_min', min(loc_belt))
				rospy.set_param('/optic_locn/belt_min', min(loc_belt))
